...
**LeetCode 终于有调试器了。** 自动分析你的 Java 题解 → 插桩编译 → 真实运行 → 每一步的变量、数据结构、调用栈全都在终端里可视化。不用浏览器、不用复制粘贴、再也不用 `System.out.println`。

## 功能

### 🔍 知识库
- 按题号秒查（`lh 94`）或关键词搜索（`lh -s 二叉树`）
- 一览全表（`lh -l`）
- 提示 / 答案 / 扩展按需显示（`-i` `-a` `-e`）

### 🐛 算法调试器（核心功能）
- **一键启动**：`lh 206 -t` 自动分析→插桩→编译→运行，全程本地，零浏览器
- **逐步执行**：`→`/`←` 逐行走代码，每步变量变化一目了然
- **数据结构 diff**：变化的节点用 `*` 标注 — 一眼看出哪里变了
- **8 种可视化**：数组、链表（带 `↓cur` 指针标注）、二叉树（`/` `\` 分支）、哈希表、栈、队列、滑动窗口、DP 热力图
- **断点系统**：`b` 设断点，`c` 执行到断点，和 gdb 一样
- **变量搜索**：`/` 搜索并高亮变量名
- **调用栈面板**：递归调用一目了然 — `dfs(left) → dfs(left) → dfs(left)`
- **秒级回放**：缓存后的 trace 秒开 — 换个输入参数才需要重跑

### 🎨 打磨
- `theme.toml` 配色方案，支持 `#RRGGBB` 十六进制 TUI 颜色
- `~/.lhconfig.toml` 全局配置（默认主题、缓存目录、JDK 路径）
- bash / zsh / fish / powershell Shell 补全
- 管道友好：自动检测非终端输出，剥离 ANSI 转义码
- 静态链接二进制，除 JDK 外零运行时依赖
